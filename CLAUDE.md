...
├── output/
│   └── README.md
├── tools/              ← Script Node kiểm thử bộ tri thức Trợ lý GĐV
│   ├── kb-env.js       ← Nạp KB + cắt engine khỏi index.html, chạy trong vm
│   ├── check-kb.js     ← Toàn vẹn dữ liệu + không lộ email/SĐT
│   ├── test-search.js  ← tlSearch trả đúng mục
│   ├── compare-search.js ← So trước/sau khi sửa, phát hiện hồi quy
│   └── README.md
├── docs/
│   ├── Quy_trinh.md    ← Đặc tả logic báo cáo
│   ├── Quy_trinh.docx
│   └── Kho tri thức/   ← Bản gốc quy trình, quy tắc, công văn, văn bản pháp luật PTI
│                         (nguồn của assets/tro-ly-gdv-kb.js; ~563 MB, phần lớn PDF scan;
│                          đã .gitignore — tài liệu nội bộ, KHÔNG đẩy lên GitHub Pages)
├── .claude/
...
- **Bộ tri thức**: `assets/tro-ly-gdv-kb.js`, nạp bằng `<script src>` ngay trước script chính. Khai báo `TL_META`, `TL_GROUPS` (13 nhóm — thêm `phaply` "Khung pháp lý" và `tinhhuong` "Tình huống thực tế" 09/2026), `TL_FORMS` (73 biểu mẫu BM.PTI.XCG.20.xx + NVXE039.01.xx), `TL_SLA_B4` (thời hạn Bước 4 theo số tiền), `TL_STATUS` (trạng thái phần mềm → bước quy trình + việc cần làm), `TL_KB` (151 mục). Thiếu file → trợ lý vẫn tra được hồ sơ.
- **Nguồn nội dung (ưu tiên quy trình PTI)** — toàn bộ nằm trong `docs/Kho tri thức/`: QT giải quyết YCBT PTI.XCG.20 (QĐ 20/12/2024) + phụ lục PL.PTI.XCG.20.01–20.18; Quy tắc VCX ô tô QĐ 109/QĐ-PTI ngày 23/09/2025 (hiệu lực 14/10/2025) + HD triển khai T10/2025; Quy tắc QĐ 110/QĐ-PTI ngày 23/09/2025 (trách nhiệm chủ xe/lái xe với phụ xe, người ngồi trên xe) + PL01 bảng tỷ lệ trả tiền thiệt hại về người; Quy trình thu hồi & thanh lý tài sản sau bồi thường (01/07/2025); CV 4406 (cam kết CLDV), CV 3653 (ảnh cấp đơn), CV 1959/PC-PTI 22/05/2025 (thanh toán phí), CV 5884; sổ tay nội bộ "Tiện ích tra cứu CV" (mở logic iLead, khuyến nghị KTNB, sai phạm cấp đơn, mốc 19/07/2024, ấn chỉ & quy tắc áp dụng, chứng từ thiệt hại tài sản NT3); cam kết dịch vụ PTI Quảng Ninh 2019; **văn bản pháp luật**: Luật KDBH 08/2022 + Luật 139/2025 (dùng bản hợp nhất 31/VBHN-VPQH), NĐ 67/2023, NĐ 220/2026, BLDS 91/2015; **sổ tay tham khảo** (gắn `v:0`): Bộ Q&A 300 tình huống + "Nhóm tình huống thực tế GĐBT bảo hiểm xe cơ giới" → nhóm `tinhhuong` 15 mục. Văn bản ban hành trước QT PTI.XCG.20 gắn `v:0`.
- ⚠️ **Chỉ dùng tài liệu còn trong `docs/`.** 09/2026 người dùng thay `docs/Thi dinh ky/` bằng `docs/Kho tri thức/` → đã gỡ 6 mục trích từ công văn không còn bản gốc (`cl-app` CV 2186, `cl-hdkh` CV 2479, `ct-xacnhan5tr` CV 87, `gd-nganhang` CV 5602, `td-nguoi-xacminh` CV 756, `td-xemay` CV 2299) và dọn mọi tham chiếu `r:` tới chúng; `gd-thanhtoanphi` được **dựng lại từ CV 1959** thay cho CV 1933 cũ. Trước đó (cùng 09/2026) đã gỡ `dg-mautrinh`, `sb-baocao` khi xoá `docs/Tien ich/` và `Mau bieu-GD/` — riêng `sb-mologic` nay **khôi phục** vì nội dung mở logic đã có lại trong "Tiện ích tra cứu CV". Không thêm lại mục nào khi chưa có tài liệu gốc. Bản gốc (phần lớn là PDF scan) nằm trong `docs/` — **không** có trên trang web.
- **Lưu ý pháp lý đã đối chiếu bản gốc (đừng suy diễn lại)**: NĐ 220/2026/NĐ-CP (22/06/2026, hiệu lực 01/07/2026) **chỉ sửa phần bảo hiểm công trình xây dựng** của NĐ 67/2023 (Điều 32–38, Phụ lục III, Mẫu số 3 PL X); với XCG nó chỉ **đổi thuật ngữ "người thứ ba" → "bên thứ ba"** (Điều 9) — mức trách nhiệm, phạm vi, loại trừ, tạm ứng, hồ sơ TNDS **không đổi**. Luật KDBH **không có "thời hiệu 3 năm"**; mốc trong Luật là **Điều 30 – nộp hồ sơ YCBT 01 năm** và **Điều 31 – trả tiền 15 ngày nếu HĐ không thoả thuận, chậm trả phải trả lãi**.
...
- **Bổ sung nội dung**: thêm mục vào `TL_KB` (id không trùng, **bắt buộc ghi `s`**), thêm từ khoá theo cách GĐV hay hỏi, rồi chạy 3 script trong **`tools/`** (xem `tools/README.md` — quy trình 5 bước):
  ```
  node tools/check-kb.js        # toàn vẹn: id, nhóm, r:, thiếu s/k, lộ email/SĐT, nguồn chết
  node tools/test-search.js     # 151/151 câu chuẩn + 32 câu diễn đạt + mục PTI thắng mục v:0
  node tools/compare-search.js  # so trước/sau (lấy bản cũ từ git) — chỉ đọc mục ③
  ```
  Mốc 26/09/2026: **151/151** câu chuẩn, 32/32 câu diễn đạt, **0 hồi quy**. Nguyên tắc: mục mới (nhất là nhóm `tinhhuong` `v:0`) lấn mục quy trình PTI thì **thêm từ khoá cho mục PTI**, đừng hạ mục mới. Hai trường hợp đã biết và **không phải hồi quy** (đã ghi trong `KNOWN` của `test-search.js`), đừng "sửa" bằng cách chỉnh mục cũ: "xe ngập nước giám định thế nào" → `tn-dacbiet` thắng `gd-ngapnuoc` (vẫn hiện `gd-ngapnuoc` ở gợi ý liên quan), và "công thức nấu phở" trúng `dg-congthuc` 8.0 vì từ khoá "công thức". `tools/kb-env.js` cắt engine khỏi `index.html` theo 2 mốc `function normText(`→`function normPlate(` và `const TL_LS_LOG=`→`function tlAsk(` — **đổi tên các hàm/hằng đó phải sửa 2 mốc này**. `tlBuildIndex` mở rộng viết tắt (`tlExpand`) cả cho câu hỏi chuẩn `_qn` — thiếu bước này, câu chuẩn có "TNDS" không bao giờ khớp nguyên văn (+100) vì câu người dùng đã được mở rộng (trước đây 3 mục `td-loaitru`, `td-nguyentac`, `td-hoso` bị `td-mtn` lấn). Giao diện điện thoại kiểm bằng Chrome headless chụp trang nhúng trong iframe 390px (Chrome desktop có độ rộng cửa sổ tối thiểu nên `--window-size=390` không mô phỏng đúng).
