...
- **Bộ tri thức**: `assets/tro-ly-gdv-kb.js`, nạp bằng `<script src>` ngay trước script chính. Khai báo `TL_META`, `TL_GROUPS` (11 nhóm — thêm `cldv` "CLDV & công văn PTI" 09/2026), `TL_FORMS` (73 biểu mẫu BM.PTI.XCG.20.xx + NVXE039.01.xx), `TL_SLA_B4` (thời hạn Bước 4 theo số tiền), `TL_STATUS` (trạng thái phần mềm → bước quy trình + việc cần làm), `TL_KB` (127 mục). Thiếu file → trợ lý vẫn tra được hồ sơ.
- **Nguồn nội dung (ưu tiên quy trình PTI)**: QT giải quyết YCBT PTI.XCG.20 (QĐ 20/12/2024) + phụ lục PL.PTI.XCG.20.01–20.18; Quy tắc VCX ô tô QĐ 109/QĐ-PTI ngày 23/09/2025 (hiệu lực 14/10/2025); Quy tắc QĐ 110/QĐ-PTI ngày 23/09/2025 (trách nhiệm chủ xe/lái xe với phụ xe, người ngồi trên xe); NĐ 67/2023/NĐ-CP (trích theo PL.20.12); công văn nghiệp vụ trong `docs/Thi dinh ky` (CV 4406 cam kết CLDV, 2186 App GĐV, 3653 ảnh cấp đơn, 1933 thanh toán phí, 5884, 5602, 2479, 756, 2299, 87 — văn bản ban hành trước QT PTI.XCG.20 gắn `v:0`); luồng hủy HS. ⚠️ **Chỉ dùng tài liệu còn trong `docs/`**: 09/2026 người dùng đã xoá thư mục `Tien ich/` (mẫu trình giá, hướng dẫn mở logic iLead, HDSD báo cáo), file "Thống nhất phối hợp PTI SOS – ĐVKD 2026" và `Mau bieu-GD/` → đã gỡ 3 mục (`dg-mautrinh`, `sb-mologic`, `sb-baocao`) và mọi câu/nguồn trích từ các file đó; không thêm lại khi chưa có tài liệu gốc. Bản gốc (phần lớn là PDF scan) nằm trong `docs/` — **không** có trên trang web.
